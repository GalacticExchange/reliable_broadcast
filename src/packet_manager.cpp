...
void PacketManager::asyncProcess(std::shared_ptr<AbstractMessage> message) {
    mIoService.post([this, message]() {
        if (message->getType() <= AbstractMessage::MessageType::READY) {
            ...
            for (auto packetedMessage :
                    dynamic_pointer_cast<PacketMessage>(message)->getMessages()) {
...
void PacketManager::setNodes(vector<Node> nodes) {
...
void PacketManager::addNode(const Node &n) {
    auto pair = std::pair<int, Node>(n.getId(), n);
    this->nodes.insert(pair);
//    this->nodes[n.getId()] = n;
}


void PacketManager::updateQueues() {

    for (ChainConfig &chainConfig : mNodeConfig.getConfigs()) {
        for (const auto &idNode : chainConfig.getNodes()) {
            // initializing queues
            queues[idNode.first];

            addNode(idNode.second);
        }
    }
